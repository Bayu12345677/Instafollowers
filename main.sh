...
res = s.post('https://instagram.qlizz.com/send', headers=headers, data=data).text
	...
	while true; do
		...
		sleep 01
		printf "$up"
		if ((hframe == 1)); then var chwr : "${me}"; var chwt : "${ij}"; var wwrh : "$cy"; var wwri : "${ku}"
		elif ((hframe == 2)); then var chwr : "${ij}"; var chwt : "${me}"; var wwri : "$cy";var wwrh : "$ku"; let hframe=0
		fi
		Tulis.str "\r${ku}[${chwr}•${chwt}•${ku}]${pu} mengisi ulang point ${m}[${wwrh}${menit}${me}:${wwri}${detik}${m}] ${st}"
		printf "$down"
		if ((menit == 0)); then
			ig.initialize Username="$tumb" pass="$pwtumb" Usr="$trg"
			break
