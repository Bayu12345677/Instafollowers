...
Bash.import: minimist/args util/IO.FUNC urlib/pyreq
...
def: __main__()
{
	banner
	printf "${up}"
	Tulis.str "${me}[${pu}+${me}]${pu} Masukan Username ${ku}(${hi}Tumbal${ku})${m}>${st} "; read tumb
	printf "${down}${clear}"
	printf "$up"
	Tulis.str "${me}[${pu}+${me}]${pu} Masukan password ${ku}(${hi}Akun tumbal${ku})${m}>${st} "; read pwtumb
	printf "${down}${clear}"
	printf "$up"
	Tulis.str "${me}[${pu}+${me}]${pu} Masukan Username ${ku}(${hi}Akun Ori${ku})${m}>${st} "; read trg
	echo
#	while true; do
...
#	Tulis.strN "${ku}• ${pu}Tor telah di mulai ${ij}√${st}"
	tput civis
	while true; do
		animasi.cek "${ku}• ${pu}Mencoba login"
	done &
	let sig=${!}
	printf "${clear}"
	...
	if (ambil: login in "success"); then
		Tulis.strN "${ku}•${pu} Berhasil login ${ij}√${st}"
	else
		Tulis.strN "${ku}•${pu} Gagal Login ${ku}(${hi}Pastikan password dan username akun tumbal benar dan pakaikan foto profile${ku})${st}"
		#tor.stop
		exit
	fi
		var iii : "${me}┄┄┄┄┄-┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄${ku}[\e[100m${cy}•${pu} INFORMASI ${cy}•${st}${ku}]${me}┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄${st}"
	Tulis.strN "$iii"
	while true; do
		while true; do
			animasi.cek "${ku}•${pu} Memulai menyuntikan followers${st}"
		done &
		let sig=${!}
...
res = s.post('https://instagram.qlizz.com/send', headers=headers, data=data).text
	...
	while true; do
		...
		sleep 01
		printf "$up"
		Tulis.str "\r${ku}[${me}!${ku}]${pu} Cooldown ${cy}${menit}${me}:${ij}${detik}${st}"
		printf "$down"
		if ((menit == 40)); then
			break
		fi
...
[!] perkecilkan layar anda untuk melanjutkan ke tools
	"|less; exit
